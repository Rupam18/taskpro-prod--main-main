...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function updateOverview(query) {
    try {
        const res = await fetch(`${API_BASE}/analytics/overview${query}`, { headers });
  ...
  async function updateStatusChart(query) {
    try {
        ...
        renderPieChart('statusChart', ['To Do', 'In Progress', 'Done'], [data.todo, data.inProgress, data.done]);
    } catch (err) { console.error(err); }
  }

  async function updateTimeChart() {
    try {
        const res = await fetch(`${API_BASE}/analytics/tasks-over-time`, { headers });
        const data = await res.json();
        
        renderLineChart('timeChart', data.map(d => d.date), data.map(d => d.count));
    } catch (err) { console.error(err); }
  }

  async function updateProjectProgress() {
    try {
        const res = await fetch(`${API_BASE}/analytics/project-progress`, { headers });
        const data = await res.json();
        
        renderBarChart('projectChart', data.map(d => d.name), data.map(d => d.percent), data.map(d => d.color));
    } catch (err) { console.error(err); }
  }
