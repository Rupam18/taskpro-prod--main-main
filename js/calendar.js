...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function createDayCell(date, isOtherMonth = false) {
    ...
    // Drag & Drop
    day.ondragover = (e) => { e.preventDefault(); day.classList.add('drop-target'); };
  ...
  function openEditModal(task) {
    ...
    document.getElementById('taskAssignedTo').value = task.assignedTo?._id || task.assignedTo || '';
  ...
  function setupEventListeners() {
    ...
    document.getElementById('newTaskBtn').onclick = () => openTaskModal(new Date().toISOString().split('T')[0]);
    ...
    searchInput.addEventListener('input', (e) => {
      ...
      const results = allTasks.filter(t => t.title.toLowerCase().includes(term));
